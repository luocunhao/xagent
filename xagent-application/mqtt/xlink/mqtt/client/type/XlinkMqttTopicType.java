package xlink.mqtt.client.type;
...
import xlink.mqtt.client.utils.MqttTopicMatcher;
...
public enum XlinkMqttTopicType {
  ...
  DEVICE_ONLINE("$xlink/device/{device_id}/state/online", "$3/{device_id}") {
    ...
    //  return Integer.toString(message.variableHeader().messageId());
    	return "online:"+message.variableHeader().topicName().replace("$3/", "");
    }

  },
  DEVICE_ONLINE_RESULT("$4/{device_id}", "$4/{device_id}"){

	@Override
	public String getTokenId(String clientId, MqttPublishMessage message) {
		// TODO Auto-generated method stub
		 return "online:"+message.variableHeader().topicName().replace("$4/", "");
	}
