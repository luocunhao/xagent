...
public class QuartzManager {
    ...
    public static void addJob(String jobName, Class cls, String time) {
        try {
            ...
            // 任务名，任务组，任务执行类
            JobDetail jobDetail = JobBuilder.newJob(cls).build();
            //可以传递参数
            jobDetail.getJobDataMap().put("param", "railsboy");
            // 触发器
            Trigger trigger = TriggerBuilder.newTrigger().withIdentity(jobName, TRIGGER_GROUP_NAME).withSchedule(CronScheduleBuilder
                    .cronSchedule(time)).startNow().build();
            // 触发器时间设定
